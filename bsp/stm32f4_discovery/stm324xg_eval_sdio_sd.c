...
/*
 * Including modifications taken from:
 * https://my.st.com/public/STe2ecommunities/mcu/Lists/cortex_mx_stm32/DispForm.aspx?ID=21134&Source=/public/STe2ecommunities/mcu/Tags.aspx?tags=freertos
 * https://my.st.com/public/STe2ecommunities/mcu/Lists/cortex_mx_stm32/DispForm.aspx?ID=17428&Source=/public/STe2ecommunities/Tags.aspx?tags=sdio
 */
...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  SDDMA_InitStructure.DMA_BufferSize = BufferSize; //fixme BufferSize
...
SD_Error SD_WaitReadOperation(void)
{
    ...
    while ((DMAEndOfTransfer == 0x00) && (TransferEnd == 0) && (TransferError
            == SD_OK) && (timeout > 0)) {
        timeout--;
    }
...
SD_Error SD_Erase(uint64_t startaddr, uint64_t endaddr)
{
    ...
    while ((delay > 0) && (errorstatus == SD_OK) && ((SD_CARD_PROGRAMMING
            == cardstate) || (SD_CARD_RECEIVING == cardstate))) {
        errorstatus = IsCardProgramming(&cardstate);
        delay--;
    }
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
    ...
    *(pscr + 1) = ((tempscr[0] & SD_0TO7BITS) << 24) | ((tempscr[0]
            & SD_8TO15BITS) << 8) | ((tempscr[0] & SD_16TO23BITS) >> 8)
            | ((tempscr[0] & SD_24TO31BITS) >> 24);

    *(pscr) = ((tempscr[1] & SD_0TO7BITS) << 24) | ((tempscr[1] & SD_8TO15BITS)
            << 8) | ((tempscr[1] & SD_16TO23BITS) >> 8) | ((tempscr[1]
            & SD_24TO31BITS) >> 24);
