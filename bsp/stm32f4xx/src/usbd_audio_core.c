...
//int32_t IsocOutBuffCount = 0;
//uint32_t IsocOutBuffCountOvf = 0;
                                 ...
                                 uint8_t cfgidx)
{
  ...
  if (AUDIO_OUT_fops.Init(USBD_AUDIO_FREQ, 0, 0) != USBD_OK)
  {
    return USBD_FAIL;
  }
...
/**
  ...
  * @retval status
  */
static unsigned long long ovf = 0;
static unsigned long long tot = 0;
extern AudioBuffer_Typedef buffers[AUDIO_BUFFER_COUNT];
static uint8_t  usbd_audio_DataOut (void *pdev, uint8_t epnum)
{
...
//      return USBD_FAIL;
    ...
    buffer->sampling_freq = USBD_AUDIO_FREQ;
    buffer->size = USBD_GetRxCount(pdev, epnum) / 2;
    assert_param(USBD_GetRxCount(pdev, epnum) % 4 == 0);
    assert_param(buffer->size <= AUDIO_BUFFER_MAX_SIZE);
    if (AudioBuffer_GetFullCount() + 5 < AUDIO_BUFFER_COUNT)
    {
...
//    if ((PlayFlag == 0) && (audio_buffer_get_full_count() >= ))
//    {
...
static uint8_t  usbd_audio_SOF (void *pdev)
{
  ...
  if (PlayFlag)
  {
    ...
    if (AudioBuffer_TryGetConsumer() == NULL)
    {
